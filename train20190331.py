...
def train():
    with tf.device('/cpu:0'):
        train_text, train_y, train_e1, train_e2, train_pos1, train_pos2, train_rw, train_rw_pos, train_rw_cate = data_helpers.load_data_and_labels(FLAGS.train_path)
    with tf.device('/cpu:0'):
        test_text, test_y, test_e1, test_e2, test_pos1, test_pos2, test_rw, test_rw_pos, test_rw_cate = data_helpers.load_data_and_labels(FLAGS.test_path)
    ...
    train_rw_pos_x = np.array(list(vocab_processor2pos.transform(train_rw_pos)))
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if FLAGS.embeddings == "word2vec":
                ...
                pretrain_rw_W = utils.load_glove('resource/glove.840B.300d.txt', FLAGS.embedding_size, vocab_processor2)
                sess.run(model.W_text.assign(pretrain_W))
            ...
            train_batches = data_helpers.batch_iter(list(zip(train_x, train_y, train_text,
                                                             ...
                                                             train_rw_x, train_rw_text,
                                                             train_rw_pos_x, train_rw_pos_text,
                                                             train_rw_cate)),
                                                    FLAGS.batch_size, FLAGS.num_epochs)
            # Training loop. For each batch...
            best_f1 = 0.0  # For save checkpoint(model)
            for train_batch in train_batches:
                train_bx, train_by, train_btxt, train_be1, train_be2, train_bp1, train_bp2, train_brw_x, train_brw_text,train_brw_pos_x, train_brw_pos_text, train_brw_cate = zip(*train_batch)
                feed_dict = {
                ...
                if step % FLAGS.evaluate_every == 0:
                    ...
                    test_batches = data_helpers.batch_iter(list(zip(test_x, test_y, test_text,
                                                                    ...
                                                                    test_rw_x, test_rw_text,
                                                                    test_rw_pos_x, test_rw_pos_text,
                                                                    test_rw_cate)),
                    ...
                    for test_batch in test_batches:
                        test_bx, test_by, test_btxt, test_be1, test_be2, test_bp1, test_bp2, test_brw_x, test_brw_text, test_brw_pos_x, test_brw_pos_text, test_brw_cate = zip(*test_batch)
                        feed_dict = {
                            ...
                            model.input_rw_pos_text: test_brw_pos_text,#######
